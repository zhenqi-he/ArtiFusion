...
class KNN:
    def __init__(
        self, dim, max_num_entries, cap_num_entries=False, M=15, keep_stats=False
    ):
    ...
    def add(self, x, ids):
        ...
        if self.keep_stats:
            ...
            self.age_num_iterations = np.concatenate(
                (np.zeros_like(ids), self.age_num_iterations)
            )
            self.ages_since_last_hit = np.concatenate(
                (np.zeros_like(ids), self.ages_since_last_hit)
            )
...
class KNNMemory:
    def __init__(
        ...
        self.db = np.memmap(
            memmap_filename, mode="w+", dtype=np.float32, shape=self.shape
        )
        self.knns = [
            KNN(dim=dim, max_num_entries=max_memories, cap_num_entries=True)
        ...
        # support EMA adding
        # self.ema_adding = ema_adding
        # self.ema_adding_coefficient = ema_adding_coefficient

    def set_scoped_indices(self, indices):
        indices = list(indices)
        assert all_el_unique(
            indices
        ), f"all scoped batch indices must be unique, received: {indices}"
        assert all(
    ...
    def add(self, memories):
        ...
        self.db[
            rearrange(np.array(self.scoped_indices), "i -> i 1"), add_indices
        ] = memories
        # else:
        #     # EMA Updating
        #     self.db[
        #         rearrange(np.array(self.scoped_indices), "i -> i 1"), add_indices
        #     ] = self.ema_adding_coefficient * memories + (1-self.ema_adding_coefficient) * (1-self.db[rearrange(np.array(self.scoped_indices), "i -> i 1"), add_indices])
    ...
    def search(self, queries, topk, nprobe=8, increment_hits=True, increment_age=True):
        ...
        fetched_indices = Parallel(n_jobs=self.n_jobs)(
            knn_search(*args) for args in zip(knns, queries)
        )
        ...
        all_key_values = all_key_values.masked_fill(
            ~rearrange(all_masks, "... -> ... 1 1"), 0.0
        )

        all_key_values = rearrange_with_anon_dims(
            all_key_values, "b (...p) ... -> b ...p ...", p=prec_dims
        )
        all_masks = rearrange_with_anon_dims(
            all_masks, "b (...p) ... -> b ...p ...", p=prec_dims
        )
...
class KNNMemoryList(list):
                ...
                [
                    KNNMemory(
                        *args,
                        ...
                        memmap_filename=str(
                            memories_path / f"knn.memory.layer.{ind + 1}.memmap"
                        ),
                        **kwargs,
                    )
    ...
    @contextmanager
    def at_batch_indices(self, indices):
        knn_batch_indices_contexts = [
            memory.at_batch_indices(indices) for memory in self
        ]
